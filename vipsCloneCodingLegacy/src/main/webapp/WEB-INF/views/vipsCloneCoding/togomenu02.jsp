<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>픽업&딜리버리</title>
<script src='/resources/scripts/tempCommon.js'></script>
<link rel='stylesheet' href='/resources/styles/togomenu02.css' />
</head>
<body>
	<div id="root">
				<div class="headerLocation"><jsp:include page="template/templateHeaderFirst.jsp"></jsp:include></div>
				<div class="mainContent02">
					<div class="sidebar02">
						<div class="sidebarMain">
							<a href="/mainSaladbar">
								<img src="/resources/images/templet_saladMain/h3_sallad.gif">
							</a>
						</div>
						<ul class="sidebarList">
							<li>
								<a href="/saladbar">
									<img src="/resources/images/templet_saladMain/lnb03_01.gif">
								</a>
							</li>
							<li>
								<a href="/saladBeverage01">
									<img src="/resources/images/templet_saladMain/lnb03_02.gif">
								</a>
							</li>
							<li>
								<a href="/togomenu01">
									<img src="/resources/images/templet_togomenu/픽업&딜리버리.gif">
								</a>
							</li>
							<li>
								<a href="/ingredientInfo">
									<img src="/resources/images/templet_saladMain/lnb03_04.gif">
								</a>
							</li>
							<li>
								<a href="/allergy">
									<img src="/resources/images/templet_saladMain/lnb03_05.gif">
								</a>
							</li>
							<li>
								<a href="/allergyDelivery">
									<img src="/resources/images/templet_saladMain/lnb03_07.gif">
								</a>
							</li>
						</ul>					
					</div>
					<div class="content02">
						<div class="content-top02">
							<div class="path02">
								<a href="https://www.ivips.co.kr/intro/230918/intro.asp">
									<img src="/resources/images/templet_saladMain/ico_home.png">
								</a>
								<a href="https://www.ivips.co.kr/intro/230918/intro.asp" class="link01">Home</a>
								<a href="/mainSaladbar" class="arrow01">
									<img src="/resources/images/templet_saladMain/ico_arr.png">
								</a>
								<a href="/mainSaladbar" class="link02">SALAD BAR</a>
								<span class="arrow02">
									<img src="/resources/images/templet_saladMain/ico_arr.png">
								</span>
								<span class="link03">픽업&딜리버리</span>
							</div>
							<div class="contentText">
								<div class="text01">								
									<img src="/resources/images/templet_togomenu/픽업&딜리버리 메뉴.png">
								</div>
								<div class="text02">
									<img src="/resources/images/templet_togomenu/text02.png">
								</div>
							</div>
							<div class="contentImage">
								<img src="/resources/images/templet_togomenu/bg_visual08.png">
							</div>
							<div class="sns-Link" onclick="snsLink_7()">								
								<img src="/resources/images/templet_saladMain/ico_link.png">				
							</div>
						</div>
						<div class="content-bottom02">
							<ul class="content03-menus">
								<li>
									<a href="/togomenu01">
										<img src="/resources/images/templet_togomenu/빕스 얌 딜리버리 (1).gif">
									</a>
								</li>
								<li>
									<a href="/togomenu02">
										<img src="/resources/images/templet_togomenu/빕스 레스토랑 간편식 (1).gif">
									</a>
								</li>								
							</ul>
							<hr class="line">
							<div class="mainContent01">
								<div class="restaurantText">
									<img src="/resources/images/templet_togomenu/restaurantText.png">
								</div>
								<div class="restaurantMenu">
									<img src="/resources/images/templet_togomenu/레스토랑메뉴.jpeg">
								</div>
							</div>
							<div class="storeGuide">
								<img src="/resources/images/templet_togomenu/스토어안내.png">
							</div>
							<div class="storeLink">
								<a href="https://brand.naver.com/cjfoodville">
									<img src="/resources/images/templet_togomenu/스마트 스토어 바로가기.png">
								</a>
							</div>
							<div class="foodInfo">
								<div class="rmr-title">
									빕스 레스토랑 간편식(RMR) 상품 정보 제공
								</div>
								<table class="rmr-list">
									<colgroup>
										<col width="50%">
										<col width="50%">
									</colgroup>
									<tbody>
										<tr>
											<td>
												빕스 고르곤졸라 크림 뇨끼
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
											<td>
												빕스 고르곤졸라 피자
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
										</tr>
										<tr>
											<td>
												빕스 단호박 찹 스테이크
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
											<td>
												빕스 불고기 피자
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
										</tr>
										<tr>
											<td>
												빕스 소고기 쌀국수
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
											<td>
												빕스 척 아이롤 스테이크
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
										</tr>
										<tr>
											<td>
												빕스 콤비네이션 피자
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
											<td>
												빕스 페퍼로니 피자
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
										</tr>
										<tr>
											<td>
												빕스 하와이안 찹 스테이크
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
											<td>
												빕스 훈제 연어
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
										</tr>
										<tr>
											<td>
												홈파티 치즈 플래터
												<a href="">
													<img src="/resources/images/templet_togomenu/상품 정보보기.png">
												</a>
											</td>
											<td></td>
										</tr>
									</tbody>
								</table>
							</div>
						</div>
						<div class="caution">
							<div class="lookList">
								<img src="/resources/images/templet_saladMain/tit_look.png">
							</div>
							<div class="cautionList">								
								<div class="listImage">
									<img src="/resources/images/templet_saladMain/bul_4dot.png">
								</div>
								<div class="listText">
									상기 이미지는 실제 제공되는 제품과 다를 수 있습니다.
								</div>
								<div class="listImage">
									<img src="/resources/images/templet_saladMain/bul_4dot.png">
								</div>
								<div class="listText">
									제품별 가격은 구매 페이지에서, 원산지 등 상세 제품정보는 제품 패키지 및 구매 페이지에서
									확인하실 수 있으며, 원재료 수급상황에 따라 원산지는 변경될 수 있습니다.
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="contentFinish">
					<img src="/resources/images/templet_saladMain/bg_contents_bottom.gif">
				</div>
				<div class="footerLocation"><jsp:include page="template/templateFooterFirst.jsp"></jsp:include></div>
				</div>			
</body>
</html>